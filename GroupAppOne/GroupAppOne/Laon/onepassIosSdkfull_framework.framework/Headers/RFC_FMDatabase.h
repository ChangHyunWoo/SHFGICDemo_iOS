...
#import <sqlite3.h>
#import "RFC_FMResultSet.h"
#import "RFC_FMDatabasePool.h"
 

#if ! __has_feature(objc_arc)
    #define RFC_FMDBAutorelease(__v) ([__v autorelease]);
    #define RFC_FMDBReturnAutoreleased RFC_FMDBAutorelease

    #define RFC_FMDBRetain(__v) ([__v retain]);
    #define RFC_FMDBReturnRetained RFC_FMDBRetain

    #define RFC_FMDBRelease(__v) ([__v release]);

    #define RFC_FMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define RFC_FMDBAutorelease(__v)
    #define RFC_FMDBReturnAutoreleased(__v) (__v)

    #define RFC_FMDBRetain(__v)
    #define RFC_FMDBReturnRetained(__v) (__v)

    #define RFC_FMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define RFC_FMDBDispatchQueueRelease(__v)
    #else
        #define RFC_FMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 ### Usage
 The three main classes in RFC_FMDB are:

 - `RFC_FMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<RFC_FMResultSet>` - Represents the results of executing a query on an `RFC_FMDatabase`.
 - `<RFC_FMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<RFC_FMDatabasePool>` - A pool of `RFC_FMDatabase` objects.
 - `<RFC_FMStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [RFC_FMDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [RFC_FMDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `RFC_FMDatabase` object and use it across multiple threads. Instead, use `<RFC_FMDatabaseQueue>`.
...
@interface RFC_FMDatabase : NSObject  {
...
/** Create a `RFC_FMDatabase` object.
 
 An `RFC_FMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `RFC_FMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `RFC_FMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    RFC_FMDatabase *db = [RFC_FMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    RFC_FMDatabase *db     = [RFC_FMDatabase databaseWithPath:dbPath];
 ...
 @return `RFC_FMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `RFC_FMDatabase` object.
...
/** Execute single update statement
 ...
 @note If you want to use this from Swift, please note that you must include `RFC_FMDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeUpdate:withArgumentsInArray:>`.
 */
...
/** Execute select statement
 ...
 Executing queries returns an `<RFC_FMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[RFC_FMResultSet next]>`) from one record to the other.
 ...
 @return A `<RFC_FMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see RFC_FMResultSet
 @see [`RFC_FMResultSet next`](<[RFC_FMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 
 @note If you want to use this from Swift, please note that you must include `RFC_FMDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeQuery:withArgumentsInArray:>`.
 */

- (RFC_FMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (RFC_FMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (RFC_FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (RFC_FMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (RFC_FMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(RFC_FMDatabase *adb) {
        ...
        int rowCount = 0;
        RFC_FMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [RFC_FMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one RFC_FMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using RFC_FMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to RFC_FMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using RFC_FMDB you will not need to interact directly with `FMStatement`, but rather with `<RFC_FMDatabase>` and `<RFC_FMResultSet>` only.
...
@interface RFC_FMStatement : NSObject {

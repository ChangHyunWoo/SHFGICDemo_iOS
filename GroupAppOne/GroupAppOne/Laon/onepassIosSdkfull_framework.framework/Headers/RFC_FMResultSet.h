...
@class RFC_FMStatement;

/** Represents the results of executing a query on an `<RFC_FMDatabase>`.
...
@interface RFC_FMResultSet : NSObject {
    RFC_FMDatabase          *_parentDB;
    RFC_FMStatement         *_statement;
...
@property (atomic, retain) RFC_FMStatement *statement;
...
/** Create result set from `<FMStatement>`
 ...
 @param aDB A `<RFC_FMDatabase>` to be used
 
 @return A `RFC_FMResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(RFC_FMStatement *)statement usingParentDatabase:(RFC_FMDatabase*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(RFC_FMDatabase *)newDb;
